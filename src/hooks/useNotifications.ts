...
export const familyNotificationsQueryKey = (familyId?: string, limit = 30) =>
  ['family-notifications', familyId, limit] as const

export function useFamilyNotifications(familyId?: string, limit = 30) {
  return useQuery<FamilyNotification[]>({
    queryKey: familyNotificationsQueryKey(familyId, limit),
      ...
      .on(
        ...
        () => {
          void queryClient.invalidateQueries({
            queryKey: ['family-notifications', familyId],
          })
        },
      )
      .subscribe()
