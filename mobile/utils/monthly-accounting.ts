...
export function computeMonthlyAccountingWindow(
  ...
  if (cycleConfig.cycle_type === 'monthly' || followCycleWindow) {
    // Una sola rama para "la ventana ES el ciclo": monthly (siempre) y
    // rolling elegido por el dinámico (followCycleWindow). El freeze se
    // pasa INCONDICIONAL: getCurrentPayCycle lo ignora para tipos
    // rolling (no-op, ver pay-cycle.ts) y los callers dinámicos llegan
