...
```ts
// mobile/lib/haptics.ts — wrapper centralizado

import * as Haptics from 'expo-haptics';

export const haptics = {
  // Cambios de selección: tabs, segmentos, pickers, toggles
  selection: () => Haptics.selectionAsync(),

  // CTA primaria, apertura de sheet, confirmación de interacción física
  light: () => Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light),
  medium: () => Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium),

  // Resultado final de una acción del usuario
...
| Evento | Haptic |
|--------|--------|
| Cambiar tab o segmento | `selection` |
| Abrir bottom sheet | `light` |
| Confirmar guardar gasto | `success` |
| Error de validación | `error` |
| Eliminar ítem | `warning` → luego `success` al confirmar |
| Scroll hasta un ítem | ninguno |
| Cambio de valor en slider | `selection` (throttled) |
...
export const lightColors = {
  ...
  textSoft: '#8E8E93',
  primary: '#19B66B',
  primaryStrong: '#0E8C50',
  primarySurface: '#EAF9F1',
  success: '#30D158',
  warning: '#FF9F0A',
  danger: '#FF453A',
} as const;
...
export const darkColors = {
  background: '#08100B',
  backgroundElevated: '#0E1712',
  surface: '#141B17',
  surfaceMuted: '#1A241E',
  surfaceStrong: '#223129',
  border: '#24372B',
  borderStrong: '#31503D',
  text: '#F8FBF8',
  textMuted: '#C8D2CB',
  textSoft: '#8B9A91',
  primary: '#4ED585',
  primaryStrong: '#7DDEA0',
  primarySurface: '#183324',
  success: '#35D46F',
  warning: '#F3BA57',
  danger: '#F06A6A',
} as const;
...
// Usos recomendados:
// cards principales: radii.xl (28)
// botones default: radii.lg (20) o radii.md (14) según contexto
// inputs: radii.md (14)
// chips: radii.pill
```
...
export const typography = {
  // Títulos de pantalla
  titleLarge: { fontSize: 32, fontWeight: '900', letterSpacing: -0.5 },
  titleMedium: { fontSize: 24, fontWeight: '800', letterSpacing: -0.3 },

  // Valores financieros protagonistas
  heroValue: { fontSize: 36, fontWeight: '900', letterSpacing: -1 },
  metricValue: { fontSize: 22, fontWeight: '800', letterSpacing: -0.5 },

  // Labels de sección (eyebrows)
  ...
  buttonLabelSmall: { fontSize: 13, fontWeight: '600' },
} as const;
```
...
```ts
// Sombras correctas para light mode
const shadowLight = {
  shadowColor: '#000',
...
// Sombras correctas para dark mode
// En dark: menos sombra, más borde y contraste
const shadowDark = {
  shadowColor: '#000',
...
Reglas:
- No usar sombras cortas y duras (shadowRadius < 6).
...
- TanStack Query keys: https://tanstack.com/query/v4/docs/framework/react/guides/query-keys
- TanStack Query invalidation: https://tanstack.com/query/v4/docs/framework/react/guides/invalidations-from-mutations
...
function AnimatedAmount({ value }: { value: number }) {
  ...
  const text = useDerivedValue(() =>
    `$${Math.round(animatedValue.value).toLocaleString('es-AR')}`
  );

  return <AnimatedText text={text} style={styles.amount} />;
}
```
...
<Swipeable
  renderRightActions={(progress) => (
    <SwipeDeleteAction progress={progress} onDelete={handleDelete} />
  )}
  onSwipeableOpen={() => haptics.warning()}
/>
```
