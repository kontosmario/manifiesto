...
// sad, dizzy, shy, think, zen, magic, seed, wilted, cool
(function () {
  ...
  const POSES = {
    ...
    cool:    { eyes: 'closed', mouth: 'smirk', custom: 'cool', tilt: -0.05, leaves: 'perk', sway: 1, aL: (t) => 0.16 + Math.sin(t * 1.5) * 0.04, aR: (t) => 0.9 + Math.sin(t * 1.5 + 0.5) * 0.05 },
  };
  ...
  class BrotMascot extends HTMLElement {
    ...
    attributeChangedCallback(name, oldV, newV) {
      ...
      if (name === 'pose' && oldV && newV && oldV !== newV && this._animated()) {
        this._from = oldV;
        this._tSw = performance.now();
      }
      this._resize();
    ...
    _mouth(ctx, type, t, br) {
      ...
      if (type === 'open') {
        ...
        ctx.lineWidth = 2.3;
        ctx.beginPath(); ctx.moveTo(37.6, 64.4); ctx.quadraticCurveTo(42.5, 67.6, 47.4, 61.8); ctx.stroke();
      } else if (type === 'hmm') {
        ctx.lineWidth = 2.1;
    ...
    // Transición entre emotes: dissolve + anticipación (squash) con los pies anclados.
    _draw(now) {
      const SW = 420;
      ...
      if (this._from) {
        const k = Math.min(1, (now - this._tSw) / SW);
        const e = k < 0.5 ? 4 * k * k * k : 1 - Math.pow(-2 * k + 2, 3) / 2;
        this._sq = 1 - 0.11 * Math.sin(Math.PI * k);
        this._paintPose(ctx, now, this._from, 1 - e);
        this._paintPose(ctx, now, this._pose(), e);
        if (k >= 1) { this._from = null; this._sq = 1; }
        return;
      }
      this._sq = 1;
      this._paintPose(ctx, now, this._pose(), 1);
    }

    _paintPose(ctx, now, pose, alpha) {
      const t = (now - this._t0) / 1000 + this._phase;
      const H = pose === 'peek' ? 72 : 108, W = 84;
      const s = this._scale * (this._sq || 1);
      ctx.save();
      ctx.globalAlpha = alpha;
      ctx.setTransform(s, 0, 0, s, (this._canvas.width - W * s) / 2, this._canvas.height - H * s);
      this._render(ctx, t, pose);
      ctx.restore();
      ctx.globalAlpha = 1;
    }
    ...
    _render(ctx, t, pose) {
      ...
      // gafas de sol chad (wayfarer, montura gruesa) — versión prolija y simétrica
      if (P.custom === 'cool') {
        const cx = 42, gy = 55.4, sep = 9.7, cL = cx - sep, cR = cx + sep, w = 12.2, h = 8.8, rad = 2.8;
        // rounded-rect simétrico
        const rr = (x, ww, hh) => {
          const l = x - ww / 2, r = x + ww / 2, tp = gy - hh / 2, bt = gy + hh / 2;
          ctx.beginPath();
          ctx.moveTo(l + rad, tp);
          ctx.lineTo(r - rad, tp);
          ctx.quadraticCurveTo(r, tp, r, tp + rad);
          ctx.lineTo(r, bt - rad);
          ctx.quadraticCurveTo(r, bt, r - rad, bt);
          ctx.lineTo(l + rad, bt);
          ctx.quadraticCurveTo(l, bt, l, bt - rad);
          ctx.lineTo(l, tp + rad);
          ctx.quadraticCurveTo(l, tp, l + rad, tp);
          ctx.closePath();
        };
        ctx.save();
        ctx.lineJoin = 'round'; ctx.lineCap = 'round';
        // patillas simétricas hacia las hojas
        ctx.strokeStyle = '#0B0F09'; ctx.lineWidth = 3.1;
        ctx.beginPath(); ctx.moveTo(cL - w / 2 + 0.4, gy - h / 2 + 2.2); ctx.lineTo(cL - w / 2 - 9, gy - 6.2); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(cR + w / 2 - 0.4, gy - h / 2 + 2.2); ctx.lineTo(cR + w / 2 + 9, gy - 6.2); ctx.stroke();
        // lentes oscuros con reflejo limpio
        const lens = (x0) => {
          const grd = ctx.createLinearGradient(x0 - w / 2, gy - h / 2, x0 + w / 2, gy + h / 2);
          grd.addColorStop(0, '#33472A'); grd.addColorStop(0.5, '#111C0D'); grd.addColorStop(1, '#070C06');
          rr(x0, w, h); ctx.fillStyle = grd; ctx.fill();
          ctx.save(); rr(x0, w, h); ctx.clip();
          ctx.strokeStyle = 'rgba(255,255,255,0.7)'; ctx.lineWidth = 2.1;
          ctx.beginPath(); ctx.moveTo(x0 - 3.6, gy - 2.9); ctx.lineTo(x0 + 1.2, gy + 3.4); ctx.stroke();
          ctx.strokeStyle = 'rgba(255,255,255,0.3)'; ctx.lineWidth = 1.1;
          ctx.beginPath(); ctx.moveTo(x0 - 0.6, gy - 3.2); ctx.lineTo(x0 + 2.6, gy + 0.8); ctx.stroke();
          ctx.restore();
          rr(x0, w, h); ctx.lineWidth = 1.8; ctx.strokeStyle = '#040704'; ctx.stroke();
        };
        lens(cL); lens(cR);
        // barra superior gruesa continua (la seña "chad")
        ctx.strokeStyle = '#0B0F09'; ctx.lineWidth = 3.8;
        ctx.beginPath(); ctx.moveTo(cL - w / 2 + 0.4, gy - h / 2 - 0.3); ctx.lineTo(cR + w / 2 - 0.4, gy - h / 2 - 0.3); ctx.stroke();
        // puente arqueado centrado
        ctx.lineWidth = 3.1;
        ctx.beginPath(); ctx.moveTo(cL + w / 2 - 0.4, gy - h / 2 + 2.2); ctx.quadraticCurveTo(cx, gy - h / 2 - 0.4, cR - w / 2 + 0.4, gy - h / 2 + 2.2); ctx.stroke();
        ctx.restore();
      }
