-- Fix: home_snapshot() no devolvia wrapped_seen_at (ni top_expense) en
-- monthly_summaries_history. El bundle siembra el cache de control
-- intelligence en cada refetch (seedCaches dentro del queryFn), asi que
-- sin el campo el flag 'wrapped visto' se perdia y el boton del header de
-- Control reaparecia aunque el cierre ya se hubiera visto. Agregamos
-- top_expense + wrapped_seen_at al rollup del snapshot. Regenerado de
-- pg_get_functiondef + 2 lineas en el jsonb del summary.
