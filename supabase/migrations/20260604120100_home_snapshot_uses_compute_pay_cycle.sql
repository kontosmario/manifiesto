-- supabase/migrations/20260604120100_home_snapshot_uses_compute_pay_cycle.sql
--
-- Refactor: home_snapshot() now uses compute_pay_cycle() helper for the
-- cycle window. Soporta los 4 tipos de ciclo (monthly/biweekly/weekly/custom).
-- Lógica idéntica al previo para cycle_type='monthly' (default de todas
-- las familias existentes).
--
-- Cambios respecto a 20260601007000_home_snapshot_no_spend_days.sql:
--   1) Bloque inline de cycle calc (~líneas 73-99) reemplazado por
--      llamada a public.compute_pay_cycle(...).
--   2) family_finance jsonb_build_object ahora incluye las 3 columnas
--      nuevas del ciclo (cycle_type, cycle_anchor_date, cycle_length_days)
--      para que el cliente pueda renderizar / editar el modo de ciclo.
--
-- Resto del body idéntico al previo.
