...
--
-- Además: agrega la columna family_finance.usd_rate_enabled (boolean, default
-- true) — el toggle "Cotización en dólares" de Settings (per-hogar, espejo de
-- daily_budget_nudges_enabled) — y la proyecta en el MISMO slice para que el
-- cache no se reseedee sin ella (idéntico vector de bug que local_currency).

ALTER TABLE public.family_finance
  ADD COLUMN IF NOT EXISTS usd_rate_enabled boolean NOT NULL DEFAULT true;
