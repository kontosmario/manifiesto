-- Saldo del mes = plata REAL: resta solo los fijos PAGADOS (no los pendientes) +
-- el variable. Antes restaba TODOS los fijos de una (paid+reserved). Los fijos
-- pendientes siguen en el saldo (todavía no salieron de la cuenta) y bajan a
-- medida que se pagan. El CUPO diario SÍ reserva todos los fijos (usa pressure),
-- para no dejar gastar la plata de los fijos que faltan pagar. Espeja el cliente
-- (cycle-disponible.ts: availableToday = discrecional + fijos pendientes; cupo =
-- discrecional/días). Modelo owner 2026-07-01.
