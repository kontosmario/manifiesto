...
-- ─── Down ──────────────────────────────────────────────────────────
-- Down: re-aplicar la versión previa de home_snapshot definida en
-- supabase/migrations/20260507000400_cleanup_legacy_family_code.sql
-- (sección "4. home_snapshot — drop `code` from family slice").
-- Esa versión usa to_jsonb(ff.*), to_jsonb(fe.*), to_jsonb(e.*) sin
-- caps, por lo que revertir restaura el comportamiento anterior.
