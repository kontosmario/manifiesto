-- Saldo del mes con override: restar TODO el gasto variable del ciclo (var_cycle),
-- no solo el posterior a la confirmación.
--
-- Contexto: el override (current_cycle_starting_balance) es un presupuesto BRUTO
-- del ciclo — sueldo/caja + arrastre (lo setea el confirm de cobro "cobré $X", el
-- onboarding "caja en mano", o apply_reserve_decision que suma reserva al sueldo
-- bruto). NO es una foto del saldo bancario a mitad de ciclo.
--
-- Bug: el fix previo (20260630090000) usaba var_since_confirm (gasto desde
-- last_salary_confirmed_at) asumiendo semántica de "foto al confirmar". Para un
-- override BRUTO eso DESCARTA el gasto previo a la confirmación — que igual salió
-- del presupuesto. kontosmario: gastó 139.659 el 20–22/6 (ciclo arrancó 20/6,
-- confirmó 23/6); el saldo mostraba 3.704.654 cuando el correcto por caja es
-- 3.564.995 (override 6.539.107,83 − fijos 1.163.852 − var_cycle 1.810.261).
--
-- Fix: var_metrics = var_cycle SIEMPRE (con o sin override). El override solo
-- cambia el ingreso/días/proration/ahorro, NO el boundary del gasto. Espeja el
-- cliente (family-dashboard-model: variableSpentForCycleMetrics = var_cycle).
-- Mantiene el cupo gateado (daily_budget = available_today / días restantes con
-- override) de 20260630100000. Elimina var_since_confirm (queda muerto).
