-- WHAT: Re-agregar 4 keys "control layer" a home_snapshot() que
--       fueron silenciosamente dropeadas en migraciones de mayo:
--         - monthly_summaries_history (últimos 6 ciclos cerrados)
--         - category_limits (caps por categoría de la familia)
--         - velocity_today (último velocity_snapshot)
--         - advisor_signal_dismissals (dismisses del USER actual)
--
-- WHY: Cold-start del home estaba haciendo 4 round-trips paralelos
--      adicionales (~700ms cada uno) a esas tablas porque los hooks
--      cliente fetcheaban directo. Re-agregándolas a home_snapshot
--      eliminamos esos 4 round-trips. Trade-off: payload del snapshot
--      sube ~3-4 KB. A cambio el cold-start time-to-interactive baja
--      de ~750ms a ~400ms (la latencia del snapshot solo).
--
-- Compatibilidad: solo AGREGAMOS keys, no sacamos ni renombramos.
-- Cualquier cliente que ya consume home_snapshot sigue viendo todos
-- los keys actuales + ahora 4 nuevas opcionales.
...
begin
  ...
  if v_family_id is null then
    return jsonb_build_object(
      ...
      -- Control layer (re-added 2026-05-09)
  ...
  select jsonb_build_object(
    ...
    -- ─── Control layer (re-added 2026-05-09) ──────────────────────────
    -- Compact subsets — solo lo que el cliente realmente consume,
    -- evita inflar payload con by_member / top_expense.
...
-- ═══ DOWN ═══════════════════════════════════════════════════════════
-- Re-aplicar 20260512020000_home_snapshot_payload_trim.sql para
-- volver a la versión sin las 4 control keys.
