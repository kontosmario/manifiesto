...
begin
  ...
  if v_user_id is not null then
    raise notice 'apple.review@manifiestoapp.com already exists, skipping seed.';
    return;
  end if;
  ...
  insert into auth.identities (
    ...
    gen_random_uuid(),
    v_user_id,
    jsonb_build_object('sub', v_user_id::text, 'email', 'apple.review@manifiestoapp.com'),
    'email',
    v_user_id::text,
    now(), now(), now()
  );
  ...
  -- UPSERT: el trigger recompute_family_income ya pudo haber creado el row
  -- con monthly_income=0 cuando insertamos en family_members arriba. Para
  -- ser order-independent, usamos ON CONFLICT.
  ...
  on conflict (family_id) do update set
    ...
    salary_payment_day = excluded.salary_payment_day,
    last_salary_confirmed_at = excluded.last_salary_confirmed_at,
    current_cycle_anchor = excluded.current_cycle_anchor,
    daily_budget_buffer_mode = excluded.daily_budget_buffer_mode,
    daily_budget_buffer_value = excluded.daily_budget_buffer_value;
  ...
  raise notice 'Seeded Apple Review account: apple.review@manifiestoapp.com / family APPLEREV';
end $$;
