-- WHAT: Agregar `monthly_income_contribution` al payload `family_members`
--       de home_snapshot() para que el cliente pueda seedear el cache de
--       `useFamilyMembersDetail` desde el snapshot y evitar 2 round-trips
--       extra (family_members con contribución + profiles in.(...)).
--
-- WHY: El `global-advisor-action-host` (siempre montado en home) llama a
--      `useFamilyMembersDetail`, que hacía 2 fetches: uno a family_members
--      con `monthly_income_contribution` y otro a profiles con los IDs.
--      home_snapshot ya devolvía display_name y avatar_animal joined; solo
--      faltaba el campo de income contribution para poder reemplazar
--      ambos fetches con un único seed client-side. -2 requests en
--      cold-start del home.
--
-- Compatibilidad: solo AGREGAMOS un campo al objeto family_members, no
-- sacamos ni renombramos nada. Cliente nuevo lee `monthly_income_contribution`;
-- cliente viejo lo ignora.
...
-- ═══ DOWN ═══════════════════════════════════════════════════════════
-- Re-aplicar 20260514010000_home_snapshot_add_control_keys.sql para
-- volver a la versión sin monthly_income_contribution en family_members.
