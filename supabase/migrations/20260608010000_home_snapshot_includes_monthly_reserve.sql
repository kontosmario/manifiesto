-- supabase/migrations/20260608010000_home_snapshot_includes_monthly_reserve.sql
--
-- Fix: home_snapshot() ahora incluye `monthly_reserve_amount` en el
-- bloque family_finance del payload.
--
-- Bug reportado por owner: "al refrescar, el badge (Reserva: $X) desaparece".
-- Causa: el RPC enumeraba columnas explícitamente vía jsonb_build_object
-- y nunca se actualizó cuando se sumó la columna `monthly_reserve_amount`.
-- Resultado: home_snapshot seedeaba el cache de family-finance SIN la
-- reserva → chip indigo nunca aparecía después del primer paint /
-- después de cualquier pull-to-refresh que invalidara los queries.
--
...
-- 20260604120100_home_snapshot_uses_compute_pay_cycle.sql.
