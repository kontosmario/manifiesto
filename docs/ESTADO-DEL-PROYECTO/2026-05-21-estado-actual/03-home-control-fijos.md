...
| Tabs | `FijosTabs` | Filtros con **3 buckets** (2026-05-30): **Pendientes** (pending + overdue/mora), **Pagados** (paid del ciclo activo), **Próximos** (future = fijos al día con próximo en un ciclo posterior, ej trimestral pagado en abril cuando estás en mayo). Color por bucket: peach / lime / sky-muted. Eliminamos "Todos" y "Zombis". Usa `GastosFilterPill` internamente. |
...
| **FijosTabs** | ✅ LIVE | Reutiliza GastosFilterPill. **2026-05-30**: 3 buckets (Pendientes / Pagados / Próximos). |
| **FijoRow** | ✅ LIVE | ConfettiBurst + statusOverlay + catChip + TrendBadge + **botón inline "Registrar pago"** circular (36pt visual, hitSlop 8px → ~52pt efectivo) visible cuando status es `pending`/`overdue` — el primario está a 1 tap, sin necesidad de tap-to-expand (ui-ux-pro-max: primary action no escondida). Bg theme.text para pending / rojo-deep para overdue. Press scale 0.92 (más pronunciado que la card 0.98 para reafirmar el tap icon-only). Tap-to-expand reservado para detalle + acción secundaria "Editar". Wrappea `SwipeRow` internamente (`borderRadius 16`). 4 estados visuales — paid (lime), pending (schedule muted), overdue (warning rojo + texto "En mora · Nd"), future (check muted + texto "Próximo · día N"). TrendBadge con prop `variant: 'price' \| 'arrears'` — arrears agrega sufijo "int." y usa tono rojo más fuerte. |
