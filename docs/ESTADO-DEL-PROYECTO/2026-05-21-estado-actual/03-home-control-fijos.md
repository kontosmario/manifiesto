...
> Actualizado 2026-06-24 (v2 — refactor del botón Pagar): al owner no le gustaba el look tinted ni el **halo pulse** de overdue (un `withRepeat` infinito que pintaba un halo rojo creciendo/desvaneciéndose para siempre detrás del botón — "horroroso", contra el criterio "no más pulsos"). **Eliminado TODO el pulse** (sharedValue/withRepeat/haloStyle/el View del halo). Look → **neutro CTA**: pill oscuro `theme.colors.text` + texto crema `creamCard`, el mismo lenguaje que el CTA primario de la app (16:1 light / 6.5:1 dark). **Igual para `pending` y `overdue`** — la urgencia del vencido la lleva el status badge de la fila, no el botón. `InlinePayButton` ya no recibe props de color (eran derivados del accent); único movimiento = press-scale sutil.
