...
| `paid_in_arrears` | `boolean NOT NULL DEFAULT false` | true cuando el expense fue creado al registrar pago sobre un fijo VENCIDO (`next_due_on < current_date` al momento del pago). Setea el RPC `record_fixed_expense_payment`. Consume la UI para distinguir el chip "Aumento de precio" (false) vs "Incremento con intereses" (true). Agregado 2026-05-30 vía migración `20260530120000`, aplicada a prod 2026-05-30. |
...
| Tabla | Propósito |
|-------|-----------|
| `fixed_expense_payments` | registro de pago por cuota: `(fixed_expense_id, period_month) UNIQUE`. `period_month` = mes en que VENCÍA la cuota (no el mes calendario del pago — fix `20260530160000`). Columnas extra: `paid_at timestamptz` (instante real del pago), `paid_by uuid FK → auth.users`, `expense_id uuid NULL FK → expenses ON DELETE SET NULL` (link al expense generado, mig `20260530180000` — usado por `revert_fixed_expense_payment` para rollback atómico). |
...
|-----|-------|-----------|-----|
| `home_snapshot()` | `() → jsonb` | Bundle completo para Home: profile, family, finance, expenses (120 — cada expense incluye `paid_in_arrears` desde mig. `20260530120000`), fixed_expenses (100), categories, notifications (80), savings_goal, fixed_expense_payments, monthly_summaries (6 — cada summary incluye `top_expense` y `wrapped_seen_at`), category_limits, velocity_today, advisor_signal_dismissals, has_push_subscription | `authenticated` |
| `gastos_snapshot(family_id, cycle_start, cycle_end, today?, cupo_diario?, days_per_page?, timezone?)` | `(uuid, timestamptz, timestamptz, ...) → jsonb` | Bundle Gastos: hero, calendar, categories, primera_page de `gastos_expenses_paginated` (incluye `paid_in_arrears` por row desde mig. `20260530120000`), streak_row, streak_marked_days (14 días) | `authenticated` |
...
| `record_fixed_expense_payment(fixed_expense_id, amount_override?)` | `(uuid, numeric?) → uuid` | Registra pago: insert expense + insert payment + avanza next_due_on + actualiza status. Devuelve expense_id. **2026-05-30**: nuevo param opcional `p_amount_override` (mig `20260530120000`) — cuando vino del sheet `ConfirmFixedPaymentSheet` con monto editado, ese valor se usa como `price` del expense Y se persiste como nuevo `amount` base del commitment. Captura `paid_in_arrears = (next_due_on < current_date)` snapshot ANTES de avanzar y lo guarda en el expense para que la UI distinga chip "Aumento de precio" vs "Incremento con intereses". **Refinado mismo día** (mig `20260530160000`): `period_month` anchored al `next_due_on` de la cuota (no al calendar month) — evita colisiones de UNIQUE cuando se paga anticipadamente dos cuotas en el mismo mes. **Refinado v3** (mig `20260530180000`): además puebla `expense_id` en el payment row (link 1-a-1 explícito payment ↔ expense) que la RPC de revert usa para rollback atómico. | `authenticated` |
| `revert_fixed_expense_payment(payment_id)` | `(uuid) → void` | **2026-05-30** (mig `20260530180000`). Deshace un pago confirmado: borra el expense linkeado (vía `fixed_expense_payments.expense_id`), borra el payment row, retrocede `next_due_on` al `period_month + day_of_month` del payment revertido, restaura `last_paid_at` al penúltimo pago (o NULL si no había previo), decrementa `installments_paid` (kind=installment), restaura `remaining_balance += price` (kind=debt), vuelve `status` a 'active' si estaba 'completed' por última cuota. Valida sesión + membership en la familia del commitment. Usado por el botón "Revertir pago" del expand panel y por el snackbar "Deshacer" post-pago. | `authenticated` |
