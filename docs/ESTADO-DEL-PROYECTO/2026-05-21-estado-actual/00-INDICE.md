...
>
> 🆕 **Novedad post-foto (2026-05-29, parte 6) — rama `feat/swipeable-fijos`:** **Swipe-to-action en gastos fijos.** `FijoRow` ahora va envuelto en el mismo `SwipeableRow` que usan Home actividad, Gastos · Movimientos y Notificaciones — acción única "Eliminar" (danger, ícono delete) a la derecha, mismo a11y action `delete` y `isProcessing` durante el round-trip. Las acciones existentes de la card (tap-expand → pagar / editar) quedan intactas; el swipe es atajo destructivo. Misma estética cross-surface; "ajustado para cada vista" (notif = neutral "Listo" mark-read · resto = danger "Eliminar").
>
