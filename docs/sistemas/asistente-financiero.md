...
| Sparkline DoW alignment con `startOfDay(tomorrow)` (mirror del engine, no drift en boundary nocturno) | ⚠️ `forecast-sparkline.tsx` no existe en el código actual (eliminado como huérfano); lógica absorbida por `control-forecast-strip.tsx` | `control-forecast-strip.tsx` |
...
| Trust Receipt strip — `useAdvisorValueSummary` hook + `TrustReceiptStrip` component, integrado en header del asistente | ✅ shipped → ⚠️ archivos que no existen en el código actual (`use-advisor-value-summary.ts`, `trust-receipt-strip.tsx`); verificar si fueron reemplazados |
| Strip se oculta cuando `totalActions === 0` (no "te ahorré $0" en accounts vírgenes) | ✅ shipped → ⚠️ ver nota anterior |
...
| Test suite — 15 tests en `cognitive-layer.test.ts` (forecast, causal, persona, stats, ranking) — el doc original decía 26, verificado: 15 `it()` en el archivo | ✅ shipped | `tests/unit/cognitive-layer.test.ts` |
...
| Item | Bloqueante | Tipo | Prioridad |
|---|---|---|---|
| **Test suite expandido** (top 12 builders individualmente + edge cases de ranking + fusion paths + super-signal composition) | Cobertura actual: 15 tests cognitivos (verificado 2026-05-22) + tests de control-signals. Falta: tests por builder, snapshots de copy, edge-case combinations | Vitest unit + snapshot | Alta — antes del próximo refactor mayor |
...
9. [Sistema de acciones (13 kinds)](#9-sistema-de-acciones-13-kinds)
...
└───────────┬───────────┘
            ▼
┌───────────────────────────────────────────────────────────────┐
│  use-control-action-dispatcher.ts (13 kinds únicos)           │
│  · 11 originales + open-external-url + open-coach-mode + open-settings-modal │
...
## 9. Sistema de acciones (13 kinds)

> ⚠️ El doc original decía 12 kinds. Verificado contra `control-action.ts` el 2026-05-22: hay 13 kinds (`open-settings-modal` fue agregado y no estaba documentado).
...
| `open-settings-modal` | `settings` | selection | "Configurar" | n/a | ⭐ (no estaba en doc original) |
...
case 'open-coach-mode': {
  // Ruta real verificada en use-control-action-dispatcher.ts (2026-05-22)
  router.push({ pathname: '/(app)/coach/[signalId]' as any, params: { signalId: action.signalId, topic: action.topic } })
}
```

> ⚠️ El código original en este doc decía `pathname: '/(app)/asistente/coach'` — path incorrecto. La ruta real es `/(app)/coach/[signalId]` (verificado en `use-control-action-dispatcher.ts`). La ruta `app/(app)/asistente/coach.tsx` no existe.

Modo concentrado para un signal específico (hero + constituents para super-signals + CTA primaria + empty state si el signal expiró). El `topic` se usa para el título del header; el flujo multi-step de 4 pasos descrito en §18.5 es diseño backlog — no implementado.
...
~~**Archivo**: `mobile/components/control-v2/control-v2-asesor-card.tsx`~~ — **no existe en el código actual** (eliminado como huérfano, 0 imports). El diseño de la card fue absorbido por la pantalla completa `asistente-screen.tsx`. La spec de colores y layout a continuación es aspiracional/referencia de diseño.
...
- ~~**Forecast sparkline**: mini-gráfico 7d~~ → `forecast-sparkline.tsx` eliminado 2026-05-22 (huérfano, 0 imports en asistente-screen)
...
- ~~**Trust receipt strip**: encima del compose bar~~ → `trust-receipt-strip.tsx` no existe en el código actual
- Twinkling stars background (18 partículas)
- Empty state celebration cuando todas dismissed

### 18.5 — Coach Mode ⭐ NEW
**Archivo**: `mobile/screens/home/coach-mode-screen.tsx` *(el doc original decía `mobile/screens/asistente/` — path inexistente)*

Trigger: CTA `open-coach-mode` desde super-signals. *(long-press en botón mint del Home no está implementado en `home-assistant-button.tsx`)*

> ⚠️ **Implementación real vs diseño aspiracional** (verificado 2026-05-22): el Coach Mode implementado es un **scaffold single-signal** — muestra el hero de un signal específico + sus constituents (si es super-signal) + CTA primaria + empty state si el signal expiró. **El flujo multi-step de 4 pasos descrito abajo NO está implementado** — es diseño backlog para iteraciones futuras.

**Flow implementado** (v1 scaffold):
- Recibe `signalId` + `topic` por params de ruta
- Busca el signal en `useControlV2Data`
- Muestra: hero card (icono + eyebrow + título + body + chips de impacto/confianza) + constituents (si super-signal) + botón CTA primaria + empty state si expiró
- Close: botón X en top bar o `router.back()`

**Flow aspiracional / backlog** (no implementado):
...
UI estilo "stories" — fullscreen cards horizontales con swipe. *(backlog — ver §B en PENDIENTES)*
...
> ⚠️ **Directorio real vs aspiracional** (verificado 2026-05-22): los tests del asistente **NO están** en `mobile/features/insights/__tests__/` — ese directorio no existe. Los tests reales están en `tests/unit/`. Los archivos existentes del asistente son:
> - `tests/unit/cognitive-layer.test.ts` (15 tests — forecast, causal, persona, stats, ranking) ✅ shipped
> - `tests/unit/control-signals.test.ts` ✅ shipped
>
> Los archivos listados abajo son el **objetivo backlog** del test suite expandido — **no existen aún**.

```
⚠️ BACKLOG (no existen — objetivo futuro):
tests/unit/
  control-signals.builders.test.ts          # 1 describe por builder, 35 atomic builders + 3 super
  ...
  use-control-action-dispatcher.test.ts      # 13 kinds + circuit breaker
```
...
- `mobile/features/insights/control-action.ts` — `ControlAction` type discriminado (13 kinds — ver §9)
...
- ~~`mobile/features/insights/control-advisor.types.ts`~~ — no existe en el código actual
...
- `mobile/features/insights/persona.ts` — UserPersona inference *(el doc original decía `persona-inference.ts` — path incorrecto)*
- ~~`mobile/features/insights/value-tracker.ts`~~ — no existe en el código actual
- `mobile/features/insights/use-interaction-stats.ts` — hook de InteractionStats
- ~~`mobile/features/insights/use-user-persona.ts`~~ — no existe en el código actual
- ~~`mobile/features/insights/use-signal-effectiveness.ts`~~ — no existe en el código actual
- ~~`mobile/features/insights/use-advisor-value-summary.ts`~~ — no existe en el código actual

### UI components
- ~~`mobile/components/control-v2/control-v2-asesor-card.tsx`~~ — eliminado 2026-05-22 (huérfano, 0 imports)
- `mobile/screens/home/asistente-screen.tsx` — pantalla completa modal
- `mobile/screens/home/coach-mode-screen.tsx` — sesión guiada *(el doc original decía `mobile/screens/asistente/` — path inexistente)*
...
- ~~`mobile/components/control-v2/asesor-explainer-panel.tsx`~~ — no existe en el código actual
- ~~`mobile/components/control-v2/asesor-receipt-strip.tsx`~~ — no existe en el código actual
- ~~`mobile/components/control-v2/forecast-sparkline.tsx`~~ — eliminado 2026-05-22 (huérfano, 0 imports); ver `mobile/components/home/control-forecast-strip.tsx`
- ~~`mobile/components/control-v2/super-signal-card.tsx`~~ — no existe en el código actual
- `mobile/components/home/home-assistant-button.tsx` — botón mint en home
- `mobile/components/ui/swipeable-row.tsx` — wrapper genérico de swipe
- ~~`mobile/components/ui/undo-toast.tsx`~~ — no existe en el código actual
...
- ~~`app/(app)/asistente/coach.tsx`~~ — no existe en el código actual; la ruta real de coach es `app/(app)/coach/[signalId].tsx`
- ~~`app/(app)/asistente/value-log.tsx`~~ — no existe en el código actual
...
- ~~`supabase/migrations/20260501000001_asistente_threads.sql`~~ — no existe; la tabla `asistente_conversations` está en el backlog (§B), no migrada

### Tests ⭐ NEW
- `tests/unit/cognitive-layer.test.ts` — 15 tests cognitivos (forecast, causal, persona, stats, ranking) ✅ existe
- `tests/unit/control-signals.test.ts` — integration tests ✅ existe
- ~~`mobile/features/insights/__tests__/`~~ — directorio no existe; ver §25.3 para backlog de tests pendientes
...
✅ **Acciones únicas** — 13 action kinds con icono/haptic/label distintos (verificado 2026-05-22)
...
- 10 → 13 action kinds (verificado 2026-05-22: el doc v2 original contaba 12, `open-settings-modal` fue agregado y no estaba documentado)
...
**Paths verificados**: ~55 rutas de archivo checkeadas contra el working tree.

**Paths inexistentes corregidos**:
- `forecast-sparkline.tsx` → marcado como eliminado (absorbido por `control-forecast-strip.tsx`)
- `control-v2-asesor-card.tsx` → nota explícita en §18.1 que no existe
- `use-advisor-value-summary.ts`, `trust-receipt-strip.tsx` → ya marcados en el doc original; nota confirmada
- `use-user-persona.ts`, `use-signal-effectiveness.ts`, `value-tracker.ts` → ya marcados en doc original; confirmados
- `mobile/features/insights/__tests__/` → no existe; §25.3 y §26 corregidos con la ubicación real (`tests/unit/`)
- `20260501000001_asistente_threads.sql` → ya marcado en doc original; confirmado
- `app/(app)/asistente/coach.tsx` → ya marcado; code block §9.2 corregido a ruta real `/(app)/coach/[signalId]`

**Claims de estado corregidos**:
- "26 tests en cognitive-layer.test.ts" → **15 tests** (verificado)
- "12 action kinds" → **13 action kinds** (`open-settings-modal` no documentado): corregido en §2, §3, §9 título + tabla, §26, §27, §versión
- Coach Mode §18.5: flujo multi-step 4-pasos es **backlog no implementado**; §18.5 y §9.2 etiquetados
- Sparkline DoW entry en tabla P2: actualizado a reflejar eliminación del archivo

**Secciones etiquetadas como backlog**:
- §18.5 Coach Mode multi-step flow (4 steps de "stories") → claramente separado del scaffold implementado
- §25.3 test suite estructura completa → marcada como objetivo backlog (los archivos no existen aún)
- §4 `asistente_conversations` → ya en doc como opcional/P3

**Confirmado**: asistente 100% heurístico, sin LLM (`grep anthropic/openai` en `mobile/features/insights/` → 0 resultados).

**No verificado** (fuera de presupuesto o no verificable localmente):
- Estado real de los crons en Supabase remoto (pg_cron) — solo verificable via `supabase migration list`
- Conteo exacto de 39 señales distintas (la distinción recovery-hard/recovery-soft desde 1 builder hace el conteo de IDs plausible pero no auditado signal-by-signal en el código)
- Tests de RLS en §25.6 (son pseudocódigo, no tests reales en el repo)
