...
8 triggers SQL en tablas señal. Todos son **`after insert/update`** (no `before`) → el evento original se commitea sí o sí. Todos están envueltos en `exception when others then raise notice ...` → si la detección revienta, el INSERT del gasto/fijo/meta sigue.
...
```ts
// mobile/features/achievements/use-achievements.ts:160
...
- Counter "X/14 logros" en algún lugar destacado
...
- Mini-counter "5/14" en el row de "Logros" en Settings (mostraría qué tan completo está sin abrir la galería)

## Dev preview (post-launch tool)

`Settings → Desarrollo → Preview · Logros & Racha` (solo `__DEV__`):

- **Sección logros**: lista los 14 codes. Tap → dispara el modal real, sin INSERT en DB. La modal es la misma que vería un user real ganándolo — visualmente indistinguible.
