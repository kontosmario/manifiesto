...
**Fecha del doc:** 2026-05-12 (verificado 2026-05-22)
**Estado del Asistente Financiero:** 100% heurístico, determinista, en producción.
**Estado del control-advisor edge function:** EXISTE y usa Claude (`claude-sonnet-4-6`)
en `supabase/functions/control-advisor/index.ts`, desplegado, pero **NO está wired
al cliente mobile** — ningún hook llama `supabase.functions.invoke('control-advisor')`.
Es un artefacto disponible que no está integrado en el flow de prod.
**Decisión del owner sobre el Asistente:** mantenerlo heurístico por ahora. Este doc captura
...
**Decisión actual (2026-05-12, confirmado 2026-05-22):** mantenemos el Asistente heurístico.
El edge function `control-advisor` (Claude `sonnet-4-6`) existe y está desplegado pero
no está conectado al mobile — no hay ningún `supabase.functions.invoke('control-advisor')`
en el código de mobile. Este doc espera al momento donde MAU + monetización justifican
integrar LLM.
