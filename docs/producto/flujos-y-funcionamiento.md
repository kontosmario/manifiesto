...
1. carga sesión, perfil, familia y estado biométrico (probe paralelo desde SecureStore),
2. si no hay sesión y el usuario tiene biometría configurada, redirige a `/(auth)/login?autoBiometric=1`,
3. si no hay sesión sin biometría, redirige a `/(auth)/welcome`,
4. si hay sesión pero la app no fue desbloqueada biométricamente este lanzamiento (app-lock), redirige a `/(auth)/login?autoBiometric=1&lock=1`,
5. si hay sesión y el onboarding no está completo (`onboarding_completed_at` es null), redirige a `/(app)/onboarding`,
6. si hay sesión pero sin familia, redirige a `/(auth)/join`,
7. si hay sesión, onboarding completo y familia, redirige a `/(app)/(tabs)/home`.
...
- `/(app)/(tabs)/home`
- `/(app)/(tabs)/expenses`
- `/(app)/(tabs)/add` (redirige a `/(app)/add-expense`)
...
El tab `Agregar` funciona como FAB central y redirige a la ruta `/(app)/add-expense`. El tab de gastos fijos tiene label `Fijos` (no `Gastos Fijos`) en la barra de navegación.
...
3. Se consultan sesión, perfil, familia y estado biométrico en paralelo.
4. Según resultado:
   - sin sesión + biometría configurada → `/(auth)/login?autoBiometric=1`
   - sin sesión sin biometría → `/(auth)/welcome`
   - con sesión + biometría activa + app no desbloqueada → `/(auth)/login?autoBiometric=1&lock=1` (app-lock)
   - con sesión y onboarding pendiente (`onboarding_completed_at` null) → `/(app)/onboarding`
   - con sesión pero sin familia → `/(auth)/join`
   - con sesión, onboarding completo y familia → `/(app)/(tabs)/home`
...
También admite sign-in con Apple y Google.

#### Comportamiento

1. se validan datos,
2. se ejecuta sign-up (email/contraseña, Apple o Google),
3. si Supabase devuelve sesión inmediata:
   - se persisten credenciales biométricas,
   - se navega a `/(app)/onboarding` (wizard de 5 pasos — cubre familia, perfil y parámetros base),
...
1. usuario autenticado pasa por el onboarding (`/(app)/onboarding`) o, como fallback, entra a `/(auth)/join`.
2. elige crear su familia.
3. se ejecuta RPC `bootstrap_family` (sin parámetros — el backend genera el código).
4. si la familia se crea bien:
   - se seedéan categorías por defecto si no existen,
   - se invalidan queries relevantes,
   - el gate redirige a Home.

### 7.2 Unirse con código de invitación

#### Mecanismo

Usa RPCs `peek_family_invite` (previsualización) y `consume_family_invite` (unión efectiva).

#### Secuencia

1. usuario escribe el código de invitación,
2. se previsualiza la familia destino vía `peek_family_invite`,
3. el usuario confirma,
4. se ejecuta `consume_family_invite`,
5. si la unión es correcta:
   - se invalidan queries de familia, categorías y gastos,
   - el gate resuelve Home o continúa el onboarding.

### 7.3 Onboarding post signup

Después del signup, el usuario es redirigido al wizard de onboarding en:

`/(app)/onboarding`

El wizard de 5 pasos cubre creación o unión a familia, configuración de perfil y parámetros financieros base. La pantalla `/(auth)/join` se usa solo como fallback para usuarios sin familia que ya completaron el onboarding.
...
La pantalla está organizada como scroll vertical con secciones ancladas navegables:

- `hoy` — héroe principal con estado del presupuesto diario,
- `alcanza` — proyección de cierre del ciclo,
- `alcancia` — seguimiento de ahorro,
- `semana` — comparación de gasto vs semana anterior,
- `vsmes` — gasto del ciclo actual vs meses previos,
- `patron` — patrón de gasto detectado,
- `cobertura` — cobertura de compromisos fijos.

El Asistente financiero puede enviar deep-links con `?section=<anchor>` para hacer scroll directo a una sección.

### Base de cálculo

`useControlV2Data` combina:

- snapshot del hogar (vía `useControlSnapshot`),
- finanzas familiares,
- gastos variables,
- analytics de gasto,
...
Incluye presupuesto diario, gasto de hoy y estado de presión del ciclo.

### 13.2 Proyección y análisis (alcanza, alcancia, semana, vsmes, patron)

Responde:

`¿Qué me conviene ajustar para no desordenar el cierre?`

Incluye proyección del ciclo, comparativas temporales y patrón de gasto detectado.

### 13.3 Cobertura fijos (cobertura)

Responde:

`¿Cómo están cubiertos mis compromisos fijos este ciclo?`

Incluye presión de fijos sobre el ingreso disponible.
...
Permite actualizar `display_name` y avatar.

### 16.2 Hogar

Permite editar el aporte mensual, el día de cobro, la cotización USD, la meta de ahorro porcentual y el buffer diario. Los campos estructurales solo los puede editar el owner de la familia.

### 16.3 Metas de ahorro

Permite editar el objetivo de ahorro fijo en pesos (savings goal).

### 16.4 Familia

Permite invitar a alguien (genera código de un solo uso), gestionar miembros (solo si es owner), y salir o eliminar el hogar.

### 16.5 Asistente

Permite configurar preferencias del asistente financiero y restablecer tutoriales guiados.

### 16.6 Notificaciones

Permite habilitar push y configurar preferencias de notificaciones.

### 16.7 Apariencia

Permite modificar preferencia de tema (sistema / claro / oscuro).

### 16.8 Animaciones

Permite configurar la preferencia de motion decorativo (Reducir / Auto / Todas).

### 16.9 Acceso rápido

Permite activar o desactivar biometría (Face ID / huella) para el acceso rápido desde lanzamiento en frío.
...
4. el signup lo redirige a `/(app)/onboarding` (wizard de 5 pasos),
5. el wizard cubre perfil, creación o unión a familia, y parámetros financieros base,
6. al finalizar el onboarding, entra a Home,
7. empieza a cargar gastos.
