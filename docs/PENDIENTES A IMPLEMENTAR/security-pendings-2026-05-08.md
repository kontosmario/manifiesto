...
| 12 | Borrar `legacy-web-src/` | Optional | 1 comando | [§12](#12-legacy-web-src--dead-code-confirmado-listo-para-borrar) |
...
### 12. `legacy-web-src/` — DEAD CODE confirmado, listo para borrar

**Estado:** verdicto del audit: **dead code, safe to delete**
**Evidencia:** zero imports desde codebase activo, deps no instaladas (vite, ionic), excluido de typecheck/lint/tests, no hay deploy target.
**Único hallazgo:** si alguien resucitara el directorio apuntando a Supabase prod, tendrían un cliente alternativo funcional contra los mismos RPCs (zombie attack surface). Probabilidad bajísima, pero existe.
**Acción opcional (destructiva, decisión tuya):**
```
git rm -r legacy-web-src/
# y limpiar las exclusiones que ahora son innecesarias:
sed -i.bak "/legacy-web-src/d" vitest.config.ts eslint.config.js && rm vitest.config.ts.bak eslint.config.js.bak
```
Si preferís no borrar, renombralo a `_archive/web-ionic-2025/` con un README que diga "frozen, no apuntar Vite acá".
